...
public class JavaC {
	...
	/**
	 * Given a list of file-scope java code (equivalent to a .java file, including
	 * package and import declarations), compile() invokes javac to compile them,
	 * produce classfiles and return a list of <className, byte[]> pairs.
	 * 
	 * compile() dumps the source strings into their respective files, has javac
	 * compile them, then reads back the equivalent class files. The name of the
	 * source file is gleaned from the string itself; a string containing "public
	 * class Foo" is stored in tmpDir/Foo.java (where tmpDir is a temporary
	 * directory that's deleted after the compilation), and if no public class or
	 ...
	 * @return List<className,byte[]>. className is fully qualified, and byte[]
	 *         contains the bytecode of the class.
	 * @throws IOException
	 */
	public static List<ClassInfo> compile(List<String> srcCodes, File rootDir, boolean writeFiles) throws IOException {

		List<SourceInfo> srcInfos = getSourceInfos(srcCodes);

		if (rootDir == null) {
			rootDir = getTmpDir(); // something like "/tmp/kilim$2348983948"
		}
	...
	public static void main(String[] args) throws IOException {
		...
		srcCodes.add("java.lang.String");
		JavaC.compile(srcCodes, getTmpDir(), false);
	}
}